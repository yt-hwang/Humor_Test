...
const AXIS = [
  {
    ...
    left: {
      ...
      descEn: [
        "Prefers humor with pre-planned situations, lines, and timing.",
        "Finds satisfaction in structural elements like callbacks, foreshadowing, and recurring bits.",
        "Especially enjoys formats with narrative flow, like sketches or drama-style comedy.",
        "Seeks stable laughs with polished detail, rhythm, and completeness."
      ]
    },
    right: {
      code: "I",
      ...
      descEn: [
        "Laughs hardest at spontaneous one-liners and reactions in the moment.",
        "Gets a thrill from unexpected comebacks and timing reversals.",
        "Enjoys flexible formats like improv scenes and parodies.",
        "Prefers lively, reactive humor over polished, rehearsed comedy."
      ]
    },
    whatItJudges: "개그의 준비 여부에 대한 취향을 설명합니다. 사전에 설계된 개그와 현장에서 즉흥적으로 이루어지는 개그 중 어느 쪽을 더 선호하는지 알아봅니다.",
    whatItJudgesEn: "Measures your preference for prepared vs. spontaneous humor — whether you enjoy carefully crafted comedy or in-the-moment improvisation."
  },
  {
    key: "NB",
    titleKo: "개그의 맥락",
    ...
    left: {
      ...
      descEn: [
        "Enjoys humor rooted in real-life context and everyday details.",
        "Drawn to punchlines that click instantly — the 'oh, THAT!' moment.",
        "Easily immersed in scenes with clear situations, characters, and relationships."
      ]
    },
    right: {
      code: "B",
      ...
      descEn: [
        "Loves humor from twisting or breaking context with unexpected connections.",
        "Finds absurd transitions, irony, and nonsense funnier the more you think about them.",
  ...
  },
  {
    ...
    left: {
      ...
      descEn: [
        "Loves laughs that come from wordplay, rhythm, and sharp one-liners.",
        "Finds joy in spotting devices like metaphors, paradoxes, and callbacks.",
        "Gets a special thrill from a single remark that shifts the entire mood."
      ]
    },
    right: {
      code: "P",
      ko: "Physical/Expressive (비언어적)",
      en: "Physical",
      desc: [
      ...
      descEn: [
        "Enjoys nonverbal expression: facial expressions, gestures, movement, tone, costumes, and sound effects.",
        "Prefers visual performances that convey the scene without words.",
        "Easily responds to intuitive formats like short-form videos, mime, and physical comedy."
      ]
    },
    whatItJudges: "개그의 전달 방식 취향을 설명합니다. 말(언어) 중심과 표정·몸·소리 같은 비언어 중심 중 어느 쪽이 더 취향에 맞는지 알아봅니다.",
    whatItJudgesEn: "Measures your delivery preference — whether you favor word-based humor or nonverbal expression like gestures, expressions, and physicality."
  },
  {
    key: "BD",
    titleKo: "개그의 주제",
    ...
    left: {
      ...
      descEn: [
        "Enjoys warm, positive topics like everyday life, self-deprecation, and emotional empathy.",
        "Smiles first at light episodic stories anyone can enjoy.",
        "Prefers a cheerful, warm tone sprinkled with small twists and wit."
      ]
    },
    right: {
      code: "D",
      ...
      descEn: [
        "Drawn to sharp topics like satire, criticism, irony, and dark comedy.",
        "Enjoys the tension and laughter from questioning society, power, and conventions.",
        "Appreciates dense storytelling where message intensity and boundary control are alive."
      ]
    },
    whatItJudges: "개그의 주제 취향을 설명합니다. 밝고 친화적인 주제와 풍자적·비판적인 주제 중 어떤 톤의 이야기에 더 끌리는지 알아봅니다.",
    whatItJudgesEn: "Measures your thematic preference — whether you lean toward bright, friendly humor or sharp, satirical dark comedy."
  }
];
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white px-4 py-2 rounded-full text-sm font-medium mb-4">
            <span>ℹ️</span>
            <span>{t('guideTitle', lang)}</span>
          </div>
          <h1 className="text-3xl font-bold text-gray-800 mb-2">{t('guideTitle', lang)}</h1>
          <p className="text-gray-600">{t('guideSub', lang)}</p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="rounded-2xl border border-white/40 bg-white/60 p-4">
                  <div className={`inline-flex items-center gap-2 text-white text-xs font-semibold px-3 py-1 rounded-full bg-gradient-to-r ${axis.gradient} mb-3`}>
                    ...
                    <span>{lang === "en" ? axis.left.en : axis.left.ko}</span>
                  </div>
                  <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                    {(lang === "en" ? axis.left.descEn : axis.left.desc).map((d, i) => (
                      <li key={i}>{d}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="rounded-2xl border border-white/40 bg-white/60 p-4">
                  <div className={`inline-flex items-center gap-2 text-white text-xs font-semibold px-3 py-1 rounded-full bg-gradient-to-r ${axis.gradient} mb-3`}>
                    ...
                    <span>{lang === "en" ? axis.right.en : axis.right.ko}</span>
                  </div>
                  <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                    {(lang === "en" ? axis.right.descEn : axis.right.desc).map((d, i) => (
                      <li key={i}>{d}</li>
                    ))}
                  </ul>
                </div>
